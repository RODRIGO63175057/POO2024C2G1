...
import pe.edu.upeu.tienda.servicio.MenuMenuItemDao;
import pe.edu.upeu.tienda.servicio.MenuMenuItenDaoI;
import pe.edu.upeu.tienda.utils.UtilsX;
...
// Layout principal
    ...
    class MenuItemListener {
        public void handle(javafx.event.ActionEvent e) {
            ...
            if (((MenuItem) e.getSource()).getId().equals("mimimcs")) {
                System.out.println("Ejecutando acción de mimimcs");
                System.out.println("tabPaneFx: " + tabPaneFx);
                tabPaneFx.getTabs().clear();
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/mcsfx.fxml"));
                loader.setControllerFactory(context::getBean);
                Parent paneFromFXML;
                try {
                    paneFromFXML = loader.load();
                    ScrollPane dd = new ScrollPane(paneFromFXML);
                    Tab clienteTab = new Tab("Registrar MCS", dd);
                    tabPaneFx.getTabs().add(clienteTab);
                } catch (IOException ex) {
                    ex.printStackTrace();
                    throw new RuntimeException(ex);
