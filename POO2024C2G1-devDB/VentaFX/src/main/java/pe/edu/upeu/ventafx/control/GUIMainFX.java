...
import pe.edu.upeu.ventafx.dto.SessionManager;
import pe.edu.upeu.ventafx.servicio.MenuMenuItemDao;
import pe.edu.upeu.ventafx.servicio.MenuMenuItenDaoI;
import pe.edu.upeu.ventafx.utils.UtilsX;
    ...
    class MenuItemListener {
        public void handle(javafx.event.ActionEvent e) {
            ...
            if (((MenuItem) e.getSource()).getId().equals("mimiproveedor")) {
                System.out.println("Ejecutando acción de mimiproveedor");
                System.out.println("tabPaneFx: " + tabPaneFx);
                tabPaneFx.getTabs().clear();
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/RegistrarProveedorControl.fxml"));
                loader.setControllerFactory(context::getBean);
                Parent paneFromFXML;
                try {
                    paneFromFXML = loader.load();
                    ScrollPane dd = new ScrollPane(paneFromFXML);
                    Tab clienteTab = new Tab("Registrar Proveedor", dd);
                    tabPaneFx.getTabs().add(clienteTab);
                } catch (IOException ex) {
                    ex.printStackTrace();
                    throw new RuntimeException(ex);
            ...
            if (((MenuItem) e.getSource()).getId().equals("mimicliente")) {
                System.out.println("Ejecutando acción de mimicliente");
                System.out.println("tabPaneFx: " + tabPaneFx);
                tabPaneFx.getTabs().clear();
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/Registrar.fxml"));
                loader.setControllerFactory(context::getBean);
                Parent paneFromFXML;
                try {
                    paneFromFXML = loader.load();
                    ScrollPane dd = new ScrollPane(paneFromFXML);
                    Tab clienteTab = new Tab("Registrar cliente", dd);
                    tabPaneFx.getTabs().add(clienteTab);
                } catch (IOException ex) {
                    ex.printStackTrace();
                    throw new RuntimeException(ex);
